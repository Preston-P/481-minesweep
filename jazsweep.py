...
#             "coords": Label(self.frame, text = "The best coordinates to pick are: ")
#         }
...
#         self.labels["inst"].grid(row=SIZE_X+2, column=0, columnspan=SIZE_Y) #bottom bottom
...
#         self.clearSurroundingTiles(noMineID[0])

#         for x in range(0, SIZE_X):
#             for y in range(0, SIZE_Y):
#                 if self.tiles[x][y]["state"] == 1:
#                     # print(self.tiles[x][y]["coords"])
#                     # wanna take the coordinates and remove them from next possible moves to make a new list of next moves
#                     print(self.tiles[x][y])

#             # self.clearSurroundingTiles(x)

#         # Generate coordinates to pick and choose that aren't mines
...
class Minesweeper:
    ...
    def setup(self):
        ...
        for x in range(0, SIZE_X):
            for y in range(0, SIZE_Y):
                self.tiles[x][y]["heuristic"] = self.tiles[x][y]["mines"]/self.mines
    ...
    def getBest(self, tile):
        getCoords = []
        for x in range(0, SIZE_X):
            for y in range(0, SIZE_Y):
                if(tile[x][y].state == STATE_DEFAULT):
                    if(tile[x][y].mines == 1):
                        getCoords.append(tile[x][y])
        self.bestCoords = getCoords
        # return getCoords
